...
class TestInstructorEnrollsStudent(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_unenrollment_email_on(self):
        ...
        # Check the outbox
        self.assertEqual(len(mail.outbox), 3)
        self.assertEqual(mail.outbox[0].subject, 'You have been un-enrolled from Robot Super Course')
        self.assertEqual(
            mail.outbox[0].body,
            "Dear Student,\n\nYou have been un-enrolled from course Robot "
            "Super Course by a member of the course staff. "
            "Please disregard the invitation previously sent.\n\n"
            "----\nThis email was automatically sent from edx.org to "
            "student4_0@test.com"
        )
        self.assertEqual(mail.outbox[1].subject, 'You have been un-enrolled from Robot Super Course')
